...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Actualizar producto con estado
    $stmt_update_producto = $db->conexion->prepare("UPDATE productos SET nombre = ?, descripcion = ?, estado = ? WHERE id = ?");
    $stmt_update_producto->bind_param("ssii", $nombre, $descripcion, $estado, $id);
    if (!$stmt_update_producto->execute()) {
        if ($imagen) {

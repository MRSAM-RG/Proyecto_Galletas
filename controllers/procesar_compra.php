...
try {
    ...
    foreach ($carrito as $item) {
        ...
        if (!$stmt_check) {
            debug_log("Error al preparar la consulta de stock: " . $db->conexion->error);
            throw new Exception('Error al preparar la consulta de stock');
        }
        
        $stmt_check->bind_param('is', $item['producto_id'], $item['tamano']);
        if (!$stmt_check->execute()) {
            debug_log("Error al verificar el stock: " . $stmt_check->error);
            throw new Exception('Error al verificar el stock');
        }
        ...
        debug_log("Stock actual: $stock_actual");
        
        if ($stock_actual < $descontar) {
            debug_log("Error: Stock insuficiente - Actual: $stock_actual, Requerido: $descontar");
            throw new Exception('Stock insuficiente para "' . $item['nombre'] . '" (' . $item['tamano'] . ')');
        }
        
        // Descontar stock
        $stmt = $db->conexion->prepare("UPDATE stock_productos SET stock = stock - ? WHERE producto_id = ? AND tamano = ? AND stock >= ?");
        if (!$stmt) {
            debug_log("Error al preparar la actualización de stock: " . $db->conexion->error);
        ...
        if (!$stmt->execute()) {
            debug_log("Error al actualizar el stock: " . $stmt->error);
            throw new Exception('Error al actualizar el stock');
        }
        
        debug_log("Stock actualizado exitosamente para el producto ID: {$item['producto_id']}");
    }
